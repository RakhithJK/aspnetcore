...
public partial class DelegateEndpointFixer : CodeFixProvider
{
    private static async Task<Document> FixMismatchedParameterOptionality(CodeFixContext context, CancellationToken cancellationToken)
    {
        ...
        {
            var param = root.FindNode(diagnostic.Location.SourceSpan).DescendantNodes().OfType<Microsoft.CodeAnalysis.CSharp.Syntax.ParameterSyntax>().SingleOrDefault();
            if (param != null)
            {
                var newParam = param.WithType(SyntaxFactory.NullableType(param.Type));
